...
template <size_t dim>
class GradHelmholtzIfgfOperator
    : public IfgfOperator<std::complex<double>, dim, 1, GradHelmholtzIfgfOperator<dim>>
{
  public:
    typedef Eigen::Array<double, dim, Eigen::Dynamic> PointArray;
    typedef Eigen::Vector<double, dim> Point;
    GradHelmholtzIfgfOperator(std::complex<double> waveNumber, size_t leafSize, size_t order,
                              size_t n_elem = 1, double tolerance = -1)
        : IfgfOperator<std::complex<double>, dim, 1, GradHelmholtzIfgfOperator<dim>>(
    ...
    Eigen::Array<typename TX::Scalar, 1, TX::ColsAtCompileTime> d2 = x.colwise().squaredNorm();
        ...
        Eigen::Array<T,dim, TX::ColsAtCompileTime> res= x.colwise()*(factor*((-1.0 / d2) *
    Eigen::exp(- k * d) * (k + invd))).array() ; return res.array();
        //(d2 > 1e-12).select( ((-1.0 / d2) * Eigen::exp(- k * d).colwise() * (k + invd).colwise() *
    x) ,
        //                         Eigen::Array<T,
    TX::ColsAtCompileTime,dim>::Zero(x.cols(),dim)).transpose();
        }*/

    template <int dx> inline T kernelFunction(const Eigen::Ref<const Point>& x) const
    {
        double d = x.norm();
    ...
    inline void transfer_factor(TX x, TY xc, double H, TY pxc, double pH, TZ& result) const
    {
        ...
        // double d = (x - xc).norm();
        /// Eigen::Array<typename TX::Scalar, TX::ColsAtCompileTime, 1> dp = (x.colwise() -
        /// pxc).norm();
    ...
    void evaluateKernel(const Eigen::Ref<const PointArray>& x,
                        ...
                        Eigen::Ref<Eigen::Vector<T, TARGETS_AT_COMPILE_TIME>> result,
                                               ...
                                               int step = 0) const
    {
        ...
        if (step == 0)
        {
            order = (baseOrder.array() - 3)
                        .cwiseMax(2); //(baseOrder.array().template

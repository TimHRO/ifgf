...
int main(const int argc, char** argv)
{
    ...
    for (int i = 0; i <= num_splits; ++i)
    {
        ...
        double x_l = 1;
        double x_l_transformed = s_min_range + (s_max_range - s_min_range) * i / num_splits;
    ...
    auto kernel = [&](const Eigen::Vector3d& x, const Eigen::Vector3d& xp) -> T
    {
        ...
        T exp_term = std::exp(I * kappa.real() * (R_tilde - r)) * std::exp(-kappa.imag() * R_tilde);
        T result = (r / (4.0 * M_PI * R_tilde)) * exp_term;
        return result;
    };
    ...
    {
        double r = x.norm();
        return std::exp(I * kappa.real() * r) / r;
    };

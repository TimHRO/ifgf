...
#include "../core/combined_field_helmholtz_ifgf.hpp"
...
const Complex  kappa =((RealScalar) 1)*Complex(3.14*4.0,0.001);
...
{    
    ...
    const double nxy = -((x-y).dot(normal));

    // combined field: 1/(4 pi) * exp(i k d)/d^3 * ( nxy*(1 - i k d) - i k d^2 )
    auto kern = (1.0 / (4.0 * M_PI)) * (exp(I*k*norm)/(norm*norm*norm))
        * ( nxy*(1.0 - I*k*norm) - I*k*(norm*norm) );
...
//      targets.col(i)=srcs.col(i);//randomPointOnSphere(r);
 //   }
    ...
    for(int j=0;j<1;j++) {
        CombinedFieldHelmholtzIfgfOperator<dim> op(kappa,1000,8,1,-1.,-1.,-1.); //3

...
int main()
{
    ...
    Eigen::Array<double, dim, Eigen::Dynamic> points =
        Eigen::Array<double, dim, Eigen::Dynamic, Eigen::RowMajor>::Random(dim, N);
    ...
    ns.fill(p);
    // ns[0]-=2;
    Eigen::Array<double, dim, Eigen::Dynamic> interp_nodes =
        ChebychevInterpolation::chebnodesNd<double, -1, -1, -1>(ns);
    ...
    ChebychevInterpolation::parallel_evaluate<T, dim, 1>(points, transformed_values, approx_values,
                                                         ns);

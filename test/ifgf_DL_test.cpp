...
#include "../core/double_layer_helmholtz_ifgf.hpp"
...
{    
    ...
    const double xn = (x-y).dot(normal);

    // double layer: -1/(4 pi) * 1/d^2 * exp(-k d) * (-k - 1/d) * ((x-y).n)
    auto kern = -(1.0 / (4.0 * M_PI)) * (1.0/(norm*norm)) * exp(-k*norm)
        * (-k - 1.0/norm) * xn;
...
//      targets.col(i)=srcs.col(i);//randomPointOnSphere(r);
 //   }
    ...
    for(int j=0;j<1;j++) {
        DoubleLayerHelmholtzIfgfOperator<dim> op(kappa,1000,8,1,-1.,-1.,-1.); //3

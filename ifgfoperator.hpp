...
template <typename T, unsigned int DIM, unsigned int DIMOUT, typename Derived> class IfgfOperator
{
  public:
    ...
    IfgfOperator(long int maxLeafSize = -1, size_t order = 5, size_t n_elements = 1,
                 double tolerance = -1)
    {
        assert(n_elements > 0);
        if constexpr (DIM == 3)
        ...
        std::cout << "creating new ifgf operator. n_leaf=" << maxLeafSize << " order= " << order
    ...
    void init(const PointArray& srcs, const PointArray& targets, std::function<bool(double)> cutOff)
    {
        ...
        m_src_octree->calculateInterpolationRange(
            [this](double H, int step)
            { return static_cast<Derived*>(this)->orderForBox(H, m_baseOrder, step); },
            [this](double H, int step)
            {
                return static_cast<Derived*>(this)->elementsForBox(H, this->m_baseOrder,
                                                                   this->m_base_n_elements, step);
            },
    ...
    {
        std::cout << "estimating the order needed to achieve " << tol << "using "
                  << (refine == RefineH ? "h" : "p") << "-refinement" << m_baseOrder << " "
                  << m_base_n_elements.transpose() << std::endl;
        ...
        std::cout << "working on level" << level << " " << m_src_octree->numBoxes(level)
                  << std::endl;

        auto ref = tbb::parallel_reduce<tbb::blocked_range<size_t>, Eigen::Vector<size_t, DIM>>(
            tbb::blocked_range<size_t>(0, sampleBoxes), Eigen::Vector<size_t, DIM>::Zero(),
            ...
            [](const Eigen::Vector<size_t, DIM>& a,
               const Eigen::Vector<size_t, DIM>& b) -> Eigen::Vector<size_t, DIM>
            { return a.cwiseMax(b); });
    ...
    Eigen::Vector<size_t, DIM> estimateRefinementOnBox(double tol, size_t level, size_t id,
                                                       RefinementType refine)
    {
        ...
        while (error.maxCoeff() > tol && base.maxCoeff() < maxR)
        {
            ...
            new_n_els =
                refine == RefineH
                    ? m_base_n_elements.array() *
                          Eigen::pow(2 * Eigen::Vector<size_t, DIM>::Ones().array(), base.array())
                    : m_base_n_elements;

            // we compute a higher order interpolation and then sum over the additional chebychev
            // coefficients to get a better understanding over the error
            const int layers = 3;
            const auto order =
                static_cast<Derived*>(this)->orderForBox(H, new_p.array() + layers + 1, 1);

            // std::cout<<"trying order"<<order<<std::endl;
            Eigen::Vector<size_t, DIM> n_els = static_cast<Derived*>(this)->elementsForBox(
                H, new_p.array() + layers + 1, new_n_els, 1);

            // std::cout<<"trying orer"<<order.transpose()<<" and "<<n_els.transpose()<<"
            // elements"<<std::endl;
            ...
            for (int d = 0; d < DIM; d++)
            {
                const double e = Util::compute_slice_norm<T, DIM, DIMOUT>(
                    trafo_data, order.template cast<size_t>(), d, layers);
                error[d] = e;
            }
    ...
    Eigen::Array<T, Eigen::Dynamic, DIMOUT>
    mult(const Eigen::Ref<const Eigen::Vector<T, Eigen::Dynamic>>& weights)
    {
        std::cout << "mult " << m_baseOrder.transpose() << std::endl;

        std::cout << "permutation" << std::endl;
        Eigen::Vector<T, Eigen::Dynamic> new_weights(weights.size());
        Util::copy_with_permutation_rowwise<T, 1>(weights.array(), m_src_octree->permutation(),
                                                  new_weights.array());
        ...
        std::cout << "base size"
                  << static_cast<Derived*>(this)
                         ->elementsForBox(hmin, m_baseOrder, this->m_base_n_elements)
                         .transpose()
                  << std::endl;
        ...
        tbb::enumerable_thread_specific<Eigen::Array<T, Eigen::Dynamic, DIMOUT>> local_result(
            result);
        ...
        std::vector<ChebychevInterpolation::InterpolationData<T, DIM, DIMOUT>> interpolationData(
        ...
        tbb::parallel_for(tbb::blocked_range<size_t>(0, m_src_octree->numBoxes(level)),
                          [&](tbb::blocked_range<size_t> r)
                          {
                              ...
                              {
                                  recursive_mult(
                                      id, level, new_weights, local_result.local(),
                                      interpolationData[m_src_octree->parentId(level, id)],
        ...
        /*for(size_t id=0; id < m_src_octree->numBoxes(level); id++)
          recursive_mult(id, level, new_weights,result,
          interpolationData[m_src_octree->parentId(level, id)], tmp_result.local(), resultMutex,
        ...
        for (; level >= 0; --level)
        {
                                  ...
                                  for (size_t i = r.begin(); i < r.end(); i++)
                                  {
                                      for (size_t boxId : m_src_octree->nearFieldBoxes(level, i))
                                      {
                                          ...
                                          static_cast<Derived*>(this)->template evaluateKernel<1>(
                                              m_src_octree->points(srcs), m_target_octree->point(i),
                                              new_weights.segment(srcs.first, nS), result.row(i),
                                              srcs);
                                      }
                                  }
                              });
            ...
            const auto& ho_chebNodes =
                ChebychevInterpolation::chebnodesNdd<double, DIM>(high_order);
            ...
            tbb::parallel_for(
                tbb::blocked_range<size_t>(0, m_src_octree->numLeafCones(level)),
                [&](tbb::blocked_range<size_t> r)
                {
#ifdef USE_NGSOLVE
                    ...
                    for (size_t i = r.begin(); i < r.end(); i++)
                    {
                        ...
                        if (!m_src_octree->hasFarTargetsIncludingAncestors(level, boxId))
                        { // we dont need the interpolation info for those levels.
                            continue;
                        }
                        ...
                        transformInterpToCart(grid.transform(ref.id(), ho_chebNodes),
                                              transformedNodes.local(), center, H);
                        interpolationData[boxId].values.middleRows(ref.memId() * stride, stride) =
                            static_cast<const Derived*>(this)->evaluateFactoredKernel(
                                m_src_octree->points(srcs), transformedNodes.local(),
            ...
            tbb::parallel_for(
                tbb::blocked_range<size_t>(0, m_src_octree->numActiveCones(level, 1)),
                [&](tbb::blocked_range<size_t> r)
                {
                    for (size_t i = r.begin(); i < r.end(); i++)
                    {
                        ...
                        const size_t stride = ho_chebNodes.cols();
                        // before we can use the interpolation data, we habe to run a chebychev
                        // transform on it

                        tmp_chebt.local().resize(stride);

                        ChebychevInterpolation::chebtransform<T, DIM>(
                            interpolationData[boxId].values.middleRows(cone.memId() * stride,
                                                                       stride),
                            tmp_chebt.local(), high_order);
                        interpolationData[boxId].values.middleRows(cone.memId() * stride, stride) =
                            tmp_chebt.local();
                    }
                });

            // interpolation Data contains the values using the coarse- high-order interpolation
            // scheme. Project to the low-order fine grid which is faster for point-evaluations. We
            // use parentInteprolationData as a termpoary buffer.
            initInterpolationData(level, 0, parentInterpolationData);
            std::cout << "reinterpolating" << std::endl;

            tbb::parallel_for(
                tbb::blocked_range<size_t>(
                    0, m_src_octree->numActiveCones(
                           level, 1)), // iterative over the few-high order cones to take full
                                       // advantage of the TP structure
                [&](tbb::blocked_range<size_t> r)
                {
#ifdef USE_NGSOLVE
                    ...
                    for (size_t i = r.begin(); i < r.end(); i++)
                    {
                        ...
                        coarseToFine(interpolationData[boxId], level, hoCone, tmp_result.local(),
                                     order, high_order, H, parentInterpolationData[boxId]);
                    }
                });
            ...
            tbb::parallel_for(tbb::blocked_range<size_t>(0, m_src_octree->numActiveCones(level, 0)),
                              [&](tbb::blocked_range<size_t> r)
                              {
                                  for (size_t i = r.begin(); i < r.end(); i++)
                                  {
                                      ...
                                      // const size_t stride=chebNodes.cols();
                                      // before we can use the interpolation data, we habe to run a
                                      // chebychev transform on it

                                      tmp_chebt.local().resize(stride);
                                      ChebychevInterpolation::chebtransform<T, DIM>(
                                          parentInterpolationData[boxId].values.middleRows(
                                              cone.memId() * stride, stride),
                                          tmp_chebt.local(), order);
                                      parentInterpolationData[boxId].values.middleRows(
                                          cone.memId() * stride, stride) = tmp_chebt.local();
                                  }
                              });
                              ...
                              [&](tbb::blocked_range<size_t> r)
                              {
                                  ...
                                  for (size_t i = r.begin(); i < r.end(); i++)
                                  {
                                      for (size_t boxId : m_src_octree->farfieldBoxes(level, i))
                                      {
                                          ...
                                          // evaluate for the cousin targets using the interpolated
                                          // data

                                          evaluateSingleFromInterp(interpolationData[boxId],
                                                                   m_target_octree->point(i),
                                                                   center, H, tmp_result.local());
...
#ifndef BE_FAST
            tbb::parallel_for(
                tbb::blocked_range<size_t>(0, m_src_octree->numActiveCones(level - 1, 1)),
                [&](tbb::blocked_range<size_t> r)
                {
                    ...
                    for (size_t i = r.begin(); i < r.end(); i++)
                    {
                        ...
                        if (!m_src_octree->hasFarTargetsIncludingAncestors(level - 1, parentId))
                        { // we dont need the interpolation info for those levels.
                            continue;
                        }

                        transformInterpToCart(pGrid.transform(parentCone.id(), ho_chebNodes),
                                              transformedNodes.local(), parent_center, pH);
                        const size_t stride = ho_chebNodes.cols();

                        // std::cout<<"pc"<<parentCone.id()<<" "<<parentCone.memId()<<"
                        // "<<nterpolationData[parentId].values.size()<<std::endl;
                        parentInterpolationData[parentId]
                            .values.middleRows(parentCone.memId() * stride, stride)
                            .fill(0);
                        for (size_t childBox : m_src_octree->childBoxes(level - 1, parentId))
                        {
                            ...
                            transferInterp(interpolationData[childBox], transformedNodes.local(),
                                           center, H, parent_center, pH, tmp_result.local());

                            parentInterpolationData[parentId].values.middleRows(
                                parentCone.memId() * stride, stride) += tmp_result.local();
                        }
                    }
                });
#else // BE_FAST
            ...
            tbb::parallel_for(
                tbb::blocked_range<size_t>(0, m_src_octree->numActiveCones(level, 2)),
                [&](tbb::blocked_range<size_t> r)
                {
                    const size_t stride = ho_chebNodes.cols();
                    tmp_result.local().resize(ho_chebNodes.cols(), DIMOUT);
                    ...
                    for (size_t i = r.begin(); i < r.end(); i++)
                    {
                        ...
                        if (!m_src_octree->parentHasFarTargets(level, cone.boxId()))
                        { // we dont need the interpolation info for those levels.
                            continue;
                        }

                        // now reinterpolate to a polar coordinate system aligned with the parent
                        // box center via point-and shoot
                        transferRotation(interpolationData[cone.boxId()], cone,
                                         parentInterpolationData[cone.boxId()].grid,
                                         center - parent_center, tmp_coordTrafo.local(), true,
                                         parentInterpolationData[childBox].order);

                        // double n=tmp_coordTrafo.local().matrix().norm();
                        // assert(n<1e5);

                        ChebychevInterpolation::chebtransform<T, DIM>(tmp_coordTrafo.local(),
                                                                      tmp_chebt.local(), order);
                        parentInterpolationData[cone.boxId()].values.middleRows(
                            cone.memId() * stride, stride) = tmp_chebt.local();
                    }
                });
            ...
            tbb::parallel_for(
                tbb::blocked_range<size_t>(0, m_src_octree->numActiveCones(level, 3)),
                [&](tbb::blocked_range<size_t> r)
                {
                    const size_t stride = ho_chebNodes.cols();
                    tmp_result.local().resize(ho_chebNodes.cols(), DIMOUT);
                    ...
                    for (size_t i = r.begin(); i < r.end(); i++)
                    {
                        ...
                        // now reinterpolate to a polar coordinate system aligned with the parent
                        // box center via point-and shoot tmp_coordTrafo.local().fill(0);
                        transferTranslation(parentInterpolationData[cone.boxId()], cone,
                                            interpolationData[cone.boxId()].grid, center, H,
                                            parent_center, pH, tmp_coordTrafo.local());

                        // double n=tmp_coordTrafo.local().matrix().norm();
                        // std::cout<<"bla "<<n<<"
                        // "<<tmp_interpolationData[cone.boxId()].values.matrix().norm()<<std::endl;

                        //((assert(n<1e5);

                        ChebychevInterpolation::chebtransform<T, DIM>(tmp_coordTrafo.local(),
                                                                      tmp_chebt.local(), order);
                        interpolationData[cone.boxId()].values.middleRows(
                            cone.memId() * stride, stride) = tmp_chebt.local();
                    }
                });
                ...
                [&](tbb::blocked_range<size_t> r)
                {
                    ...
                    for (size_t i = r.begin(); i < r.end(); i++)
                    {
                        ...
                        for (size_t childBox : m_src_octree->childBoxes(level - 1, parentId))
                        {
                            ...
                            transferRotation(interpolationData[childBox], parentCone, pGrid,
                                             center - parent_center, tmp_result.local(), false,
                                             parentInterpolationData[childBox].order);
                            static_cast<const Derived*>(this)->transfer_factor(
                                transformedNodes.local(), center, H, parent_center, pH,
                                tmp_result.local());
                            parentInterpolationData[parentId].values.middleRows(
                                parentCone.memId() * stride, stride) += tmp_result.local();
                        }
        ...
        Util::copy_with_inverse_permutation_rowwise<T, DIMOUT>(
            result, m_target_octree->permutation(), true_result);

        return true_result;
    }

    void initInterpolationData(
        size_t level, size_t step,
        std::vector<ChebychevInterpolation::InterpolationData<T, DIM, DIMOUT>>& i_data)
    {
                           ...
                           tbb::queuing_mutex* result_mutex = 0)
    {
        ...
        for (const auto& targets : targetList)
        {
            ...
            static_cast<Derived*>(this)->template evaluateKernel<-1>(
                m_src_octree->points(srcs), m_target_octree->points(targets),
                weights.segment(srcs.first, nS), tmp_result, srcs);
    ...
    void
    evaluateFarField(size_t level, long int id,
                     const Eigen::Ref<const Eigen::Vector<T, Eigen::Dynamic>>& weights,
                     Eigen::Ref<Eigen::Array<T, Eigen::Dynamic, DIMOUT>> result,
                     ...
                     Eigen::Array<T, Eigen::Dynamic, DIMOUT>& tmp_result)
    {
        ...
        for (unsigned int l = 0; l < cousinTargets.size(); l++)
        {
            ...
            evaluateFromInterp(interpolationData, m_target_octree->points(cousinTargets[l]), center,
                               H, tmp_result);
    ...
    inline void
    recursive_mult(long int id, size_t level,
                   const Eigen::Ref<const Eigen::Vector<T, Eigen::Dynamic>>& weights,
                   Eigen::Ref<Eigen::Array<T, Eigen::Dynamic, DIMOUT>> result,
                   ...
                   PointArray& transformedNodes)
    {
        ...
        auto fine_N =
            static_cast<Derived*>(this)->elementsForBox(H, m_baseOrder, m_base_n_elements, 0);
        ...
        // if we are at the leaf-level compute the interpolation data by actually interpolating the
        // true function
        if (m_src_octree->isLeaf(level, id))
        {
            ...
            for (int memId = 0; memId < ho_grid.activeCones().size(); memId++)
            {
                ...
                transformInterpToCart(ho_grid.transform(el, ho_chebNodes), transformedNodes, center,
                                      H);
                storage.values.middleRows(memId * stride, stride) =
                    static_cast<const Derived*>(this)->evaluateFactoredKernel(
                        m_src_octree->points(srcs), transformedNodes,
                        weights.segment(srcs.first, nS), center, H, srcs);
            }
        }
        else // generate the interpolation data by evaluating the children recursively
        {
            ...
            for (size_t c = 0; c < Octree<T, DIM>::N_Children; c++)
            {
                ...
                recursive_mult(c_id, level + 1, weights, result, storage, tmp_result,
                               transformedNodes);
            }
        }
        ...
        for (int memId = 0; memId < storage.grid.activeCones().size(); memId++)
        {
            ...
            ChebychevInterpolation::chebtransform<T, DIM>(
                storage.values.middleRows(memId * stride, stride), tmpData, high_order);
...
#define USE_REINTERPOLATION
        ...
        for (int memId = 0; memId < refinedData.grid.activeCones().size(); memId++)
        {
            ...
            ChebychevInterpolation::chebtransform<T, DIM>(
                refinedData.values.middleRows(memId * stride2, stride2), tmpData, order);
            refinedData.values.middleRows(memId * stride2, stride2) = tmpData;
        }
#else
        auto& refinedData = storage;
#endif
        // Now that sthe Interpolation data storage has been prepared, we can use it to evaluate the
        // field at the cousin nodes
        ...
        if (!pGrid.isEmpty())
        {
            ...
            for (int memId = 0; memId < pGrid.activeCones().size(); memId++)
            {
                ...
                transformInterpToCart(pGrid.transform(el, p_chebNodes),
                                      transformedNodes.middleCols(idx, p_chebNodes.cols()),
                                      parent_center, pH);
                idx += p_chebNodes.cols();
            }

            transferInterp(refinedData, transformedNodes, center, H, parent_center, pH, tmp_result);
            ...
            for (int memId = 0; memId < pGrid.activeCones().size(); memId++)
            {
                ...
                parentData.values.middleRows(memId * stride, stride) +=
                    tmp_result.middleRows(idx, stride);
                idx += stride;
            }
        }
        // std::cout<<"done"<<std::endl;
    ...
    void transferRotation(const ChebychevInterpolation::InterpolationData<T, DIM, DIMOUT> data,
                          ConeRef coneRef, const ConeDomain<DIM>& domain,
                          const Eigen::Ref<const Eigen::Vector<double, DIM>>& direction,
                          Eigen::Ref<Eigen::Array<T, Eigen::Dynamic, DIMOUT>> result, bool backward,
                          const Eigen::Vector<int, DIM>& target_order) const
    {
        ...
        targets.bottomRows(2) =
            ChebychevInterpolation::chebnodesNdd<double, 2>(target_order.template tail<DIM - 1>());
        ...
        auto rotation =
            Eigen::Quaternion<double>::FromTwoVectors(direction, Eigen::Vector3d({0, 0, 1}));
        ...
        std::vector<size_t> perm = Util::sort_with_permutation(
            elIds.begin(), elIds.end(), [](auto x, auto y) { return x < y; });
        ...
        while (idx < N)
        {
            ...
            // look if any of the following points are also in this element. that way we can process
            // them together
            ...
            if (el < SIZE_MAX)
            {
                ...
                ChebychevInterpolation::fast_evaluate_tp<T, DIM, DIMOUT>(
                    tmp.array().block(1, idx, 2, nb), chebNodes1d,
            ...
            // ChebychevInterpolation::parallel_evaluate<T,
            // DIM,DIMOUT>(tmp.array().middleCols(idx,nb),
            // data.values.middleRows(memId*stride,stride), result.middleRows(idx,nb), data.order);
            idx += nb;
        }
    ...
    void transferTranslation(const ChebychevInterpolation::InterpolationData<T, DIM, DIMOUT> data,
                             ...
                             Eigen::Ref<Eigen::Array<T, Eigen::Dynamic, DIMOUT>> result) const
    {
        ...
        targets.topRows(2) =
            ChebychevInterpolation::chebnodesNdd<double, 2>(data.order.template head<DIM - 1>());

        transformInterpToCart(grid.transform(coneRef.id(), targets), targets,
                              xc - Eigen::Vector3d(0, 0, (p_xc - xc).norm()), pH);
        ...
        while (idx < N)
        {
            ...
            if (el < SIZE_MAX)
            {
                ...
                ChebychevInterpolation::fast_evaluate_tp<T, DIM, DIMOUT>(
                    tmp.array().block(0, idx, 2, nb), chebNodes1d,
                    2, // axis that is of tensor product form
                    data.values.middleRows(memId * stride, stride), res_tmp, data.order);
            }
            else
            {
                res_tmp.fill(0);
    ...
    void
    coarseToFine(const ChebychevInterpolation::InterpolationData<T, DIM, DIMOUT>& interpolationData,
                 size_t level, const ConeRef& hoCone,
                 Eigen::Array<T, Eigen::Dynamic, 1>& tmp_result,
                 const Eigen::Vector<int, DIM>& order, const Eigen::Vector<int, DIM>& high_order,
                 double H0, ChebychevInterpolation::InterpolationData<T, DIM, DIMOUT>& result)
    {
#ifdef USE_NGSOLVE
        ...
        auto fine_N = static_cast<Derived*>(this)->elementsForBox(H0, this->m_baseOrder,
                                                                  this->m_base_n_elements, 0);
        ...
        {
            auto chebNodes1d =
                ChebychevInterpolation::chebnodesNdd<double, 1>(Eigen::Vector<int, 1>(order[d]));
            ...
            for (int j = 0; j < factor[d]; j++)
            {
                ...
                points[d].segment(j * chebNodes1d.size(), chebNodes1d.size()) =
                    (chebNodes1d.array() * a) + b;
            }
        }

        tmp_result.resize(Np, DIMOUT);
        const size_t ho_stride = high_order.prod();
        ChebychevInterpolation::tp_evaluate<T, DIM, DIMOUT>(
            points, interpolationData.values.middleRows(hoCone.memId() * ho_stride, ho_stride),
            tmp_result, high_order);
        ...
        for (int l = 0; l < factor[2] * order[2]; l++)
        {
            ...
            {
                for (int k = 0; k < factor[0];
                     k++) // the innermost dimension is continuous in memory so we do things blocked
                {

                    auto ho_id = hoGrid.indicesFromId(hoCone.id());

                    const size_t fine_el =
                        (ho_id[2] * factor[2] + (l / order[2])) * result.grid.n_elements(1) *
                            result.grid.n_elements(0) +
                        (ho_id[1] * factor[1] + (j / order[1])) * result.grid.n_elements(0) +
                        (ho_id[0] * factor[0] + (k));

                    // auto fine_el2=result[boxId].grid.elementForPoint(pnt);

                    // std::cout<<"fine_el"<<fine_el<<" "<<fine_el2<<" "<<hoCone.id()<<"
                    // "<<ho_id.transpose()<<" ljk:"<<l<<" "<<j<<" "<<k<<std::endl;
                    // assert(fine_el==fine_el2);

                    if (result.grid.isActive(fine_el))
                    { // if the cone is not active, discard it
                        size_t fine_memId = result.grid.memId(fine_el);
                        size_t idx_fine = (l % order[2]) * order[0] * order[1] +
                                          (j % order[1]) * order[0]; // + (k % order[0]);

                        result.values.middleRows(fine_memId * fine_stride + idx_fine, order[0]) =
                            tmp_result.middleRows(idx_coarse, order[0]);
                    }
                    idx_coarse += order[0];
                }
            }
    ...
    void transferInterp(const ChebychevInterpolation::InterpolationData<T, DIM, DIMOUT>& data,
                        ...
                        Eigen::Ref<Eigen::Array<T, Eigen::Dynamic, DIMOUT>> result) const
    {
        ...
        while (idx < N)
        {
            ...
            else
            {
                ...
                tmp.middleCols(idx, nb) = data.grid.transformBackwards(el, tmp.middleCols(idx, nb));
                ChebychevInterpolation::parallel_evaluate<T, DIM, DIMOUT>(
                    tmp.array().middleCols(idx, nb), data.values.middleRows(memId * stride, stride),
                    tmp_result.middleRows(idx, nb), data.order);
            }
            idx += nb;
        }
    ...
    void inline evaluateSingleFromInterp(
                            ...
                            Eigen::Ref<Eigen::Array<T, Eigen::Dynamic, DIMOUT>> result) const
    {
        ...
        // std::cout<<"stride"<<stride<<std::endl;
        /*	assert(data.order==7);
                const static Eigen::Vector<double,7> nodes =
        ChebychevInterpolation::chebnodes1d<double, 7>();
        ...
        for(size_t idx=0;idx<transformed.cols();idx++) {
            ...
            result(idx)=ChebychevInterpolation::evaluate_slow<T, 1, 7,
        DIM>(transformed.array().col(idx), data.values.segment(memId*stride,stride),nodes)[0]; const
        auto cf = static_cast<const Derived *>(this)->CF(targets.col(idx) - xc); result[idx] *= cf;
        }*/
        ...
        if (true)
        {
            ...
            while (idx < N)
            {
                ...
                // transformed.col(idx)=data.grid.transformBackwards(el,transformed.col(idx));
                // look if any of the following points are also in this element. that way we can
                ...
                else
                {
                    ...
                    transformed.middleCols(idx, nb) =
                        data.grid.transformBackwards(el, transformed.middleCols(idx, nb));
                    // result.row(idx)=ChebychevInterpolation::evaluate_clenshaw<T,
                    // 1,DIM,DIMOUT>(transformed.array().middleCols(idx,nb),
                    // data.values.middleRows(memId*stride,stride),  data.order);
                    ChebychevInterpolation::parallel_evaluate<T, DIM, DIMOUT>(
                        transformed.array().middleCols(idx, nb),
                        data.values.middleRows(memId * stride, stride), result.middleRows(idx, nb),
                        data.order);
                }
                idx += nb;
            }
        ...
        else
        {
            ...
            while (idx < N)
            {
                ...
                else
                {
                    ...
                    // tmp.middleCols(idx,nb)=data.grid.transformBackwards(el,tmp.middleCols(idx,nb));
                    ChebychevInterpolation::parallel_evaluate<T, DIM, DIMOUT>(
                        tmp.array().middleCols(idx, nb),
                        data.values.middleRows(memId * stride, stride),
                        tmp_result.middleRows(idx, nb), data.order, data.grid.region(el));
                }
                idx += nb;
            }
        ...
        //     transformed.col(idx)=data.grid.transformBackwards(el,transformed.col(idx));
        //     //look if any of the following points are also in this elemnt. that way we can
        //     process them together while(idx+nb<transformed.cols() &&
        //     data.grid.elementForPoint(transformed.col(idx+nb))==el) {
        // 	transformed.col(idx+nb)=data.grid.transformBackwards(el,transformed.col(idx+nb));
        // 	nb++;
        //     }
        //     ChebychevInterpolation::parallel_evaluate<T,
        //     DIM>(transformed.array().middleCols(idx,nb),
        //     data.values.segment(memId*stride,stride), result.segment(idx,nb), data.order);
        //     idx+=nb;
        // }
    ...
    inline void transformCartToInterp(const Eigen::Ref<const PointArray>& nodes,
                                      Eigen::Ref<PointArray> transformed,
                                      const Eigen::Vector<double, DIM>& xc, double H) const
    {
    ...
    inline double cutoff_limit(double H) const { return 1e-4; }

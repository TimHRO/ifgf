...
{
public:
	ConeRef(size_t level, size_t id, size_t memId, size_t boxId) : m_level(level),
																   m_id(id),
																   m_memId(memId),
																   m_boxId(boxId)
	{
...
public:
	...
	ConeDomain(Eigen::Vector<size_t, DIM> numEls, const BoundingBox<DIM> &domain) : m_numEls(numEls),
																					m_domain(domain)
	{
		if (!domain.isNull())
		{

...
template <typename T, size_t DIM> class Octree
{
  ...
  public:
    ...
    class OctreeNode
    {
      ...
      public:
        OctreeNode(std::shared_ptr<OctreeNode> parent, unsigned int level)
            : m_parent(parent), m_isLeaf(true), m_level(level)
        {
        }
        ...
        IndexRange pntRange() const { return m_pntRange; }

        void setPntRange(const IndexRange& range) { m_pntRange = range; }
        ...
        const ConeDomain<DIM>& coneDomain(int substep = 0) const { return m_coneDomain[substep]; }
        ...
        const std::shared_ptr<const OctreeNode> child(size_t idx) const { return m_children[idx]; }

        std::shared_ptr<OctreeNode> child(size_t idx) { return m_children[idx]; }

        BoundingBox<DIM> boundingBox() const { return m_bbox; }

        void setBoundingBox(const BoundingBox<DIM>& bbox) { m_bbox = bbox; }

        bool isLeaf() const { return m_isLeaf; }

        bool hasPoints() const { return m_pntRange.first != m_pntRange.second; }

        unsigned int level() const { return m_level; }

        const std::weak_ptr<const OctreeNode> parent() const { return m_parent; }
        ...
        const std::vector<IndexRange>& nearTargets() const { return m_nearTargets; }

        const std::vector<IndexRange>& farTargets() const { return m_farTargets; }
        ...
        void print(std::string prefix = "") const
        {
            ...
            // std::cout << m_bbox.min().transpose() << "   " << m_bbox.max().transpose() <<
            // std::endl;
            std::cout << m_isLeaf << std::endl;

            // std::cout<<"("<<m_pntRange.first<<" "<<m_pntRange.second<<")"<<std::endl;
            // std::cout<<" "<<m_bbox.min().transpose()<<" to
            // "<<m_bbox.max().transpose()<<std::endl;
            /*std::cout<<m_id<<" ";
            if(m_parent)
    ...
    Octree(int maxLeafSize) : m_maxLeafSize(maxLeafSize)
    {
    ...
    void build(const PointArray& pnts)
    {
        ...
        m_permutation = Util::sort_with_permutation(pnts.colwise().begin(), pnts.colwise().end(),
                                                    zorder_knn::Less<Point, DIM>(bbox));
    ...
    void buildInteractionList(
        const Octree& target_tree, std::function<bool(double)> cutOff = []() { return false; })
    {
    ...
    void
    calculateInterpolationRange(std::function<Eigen::Vector<int, DIM>(double, int)> order_for_H,
                                std::function<Eigen::Vector<size_t, DIM>(double, int)> N_for_H,
                                std::function<double(double)> smin_for_H, const Octree& target)
    {
        ...
        for (size_t level = 0; level < levels(); level++)
        {
            ...
            tbb::parallel_for(
                tbb::blocked_range<size_t>(0, numBoxes(level)),
                [&](tbb::blocked_range<size_t> r)
                {
                    for (size_t n = r.begin(); n < r.end(); ++n)
                    {
                        ...
                        for (const IndexRange& iR : farTargets)
                        {
                            ...
                            {
                                // std::cout<<"asd"<<node->boundingBox().exteriorDistance(m_targets.col(i))<<"
                                // "<<H<<std::endl;

                                const auto s = Util::cartToInterp<DIM>(target_points.col(i), xc, H);
                                assert(s[0] <= sqrt(DIM) / DIM);

                                box.extend(s); // make sure the target is in the interpolation
                                               // domain
                            }
                        }
                        ...
                        if (parent && parentHasFarTargets(node))
                        {
                            ...
                            if (!pBox.isNull())
                            {
                                // transform the parents interpolation range to the physical
                                // coordinates
                                auto cMin = Util::interpToCart<DIM>(pBox.min().array(), pxc, pH);
                                auto cMax = Util::interpToCart<DIM>(pBox.max().array(), pxc, pH);

                                // pull those physical coordinates back to the
                                // interpolation-coordinates of node
                                ...
                                auto chebNodes = ChebychevInterpolation::chebnodesNdd<double, DIM>(
                                    order_for_H(pH, 0));
                                for (size_t el : p_grid.activeCones())
                                {
                                    for (size_t i = 0; i < chebNodes.cols(); i++)
                                    {
                                        auto pnt = Util::interpToCart<DIM>(
                                            p_grid.transform(el, chebNodes.col(i)).array(), pxc,
                                            pH);
                        ...
                        if (!box.isNull())
                        {
                            // extend the box slighlty such that the boundary points are not used
                            // for interpolation
                        ...
                        // if the sources and targets are well-separated we don't have to cover the
                        // near field
                        ...
                        */
                        ...
                        {
                            oldN = N_for_H(H, 0);
                            std::cout << "n=" << N_for_H(H, 0).transpose() << "/"
                                      << N_for_H(H, 1).transpose() << std::endl;
                            std::cout << "box=" << box << " " << box.isNull() << "H=" << H
                                      << std::endl;
                        }
                        ...
                        // now we need to do the whole thing again to figure out which cones are
                        // active...
                        //  0: where do i need to compute the points directly/from the children in
                        //  order to be able to sample FF and first rotation 1: where do i need to
                        //  compute the points using  the first rotation in order to be able to
                        //  compute the translation 2: where do i need to compute the points using
                        //  the translation in order to be able to compute the second rotation
                        ...
                        for (const IndexRange& iR : farTargets)
                        {
                            ...
                            Util::cartToInterp2<DIM>(
                                target_points.middleCols(iR.first, iR.second - iR.first), xc, H,
                                s.leftCols(iR.second - iR.first).array());
                        ...
                        if (!pBox.isNull())
                        {
                            ...
                            if (parentHasFarTargets(level, n))
                            {
                                ...
                                {
                                    auto chebNodes =
                                        ChebychevInterpolation::chebnodesNdd<double, DIM>(
                                            order_for_H(H, 1));

                                    // now we add all the points used in the point-and-shoot method.
                                    // Second rotation:
                                    for (size_t el : p_grid.activeCones())
                                    {
                                        ...
                                        auto points =
                                            p_grid.rotated_points(el, chebNodes, direction, false);

                                        for (size_t i = 0; i < points.cols(); i++)
                                        {
                                            auto coneId =
                                                trans_domain.elementForPoint(points.col(i));
                                            if (coneId < SIZE_MAX)
                                    ...
                                    {
                                        auto points = trans_domain.translated_points(
                                            el, chebNodes, xc, H, pxc, pH);

                                        for (size_t i = 0; i < points.cols(); i++)
                                        {
                                            auto coneId =
                                                coarseDomain.elementForPoint(points.col(i));
                                            if (coneId < SIZE_MAX)
                                    ...
                                    {
                                        const auto direction = xc - pxc;
                                        PointArray points = coarseDomain.rotated_points(
                                            el, chebNodes, direction, true);
                                ...
                                {
                                    // We use a coarse grid with high order and a fine grid of lower
                                    // order
                                    auto HoChebNodes =
                                        ChebychevInterpolation::chebnodesNdd<double, DIM>(
                                            order_for_H(pH, 1));
                                    ...
                                    {
                                        pnts = Util::interpToCart<DIM>(
                                            p_hoGrid.transform(el, HoChebNodes).array(), pxc, pH);
                                        Util::cartToInterp2<DIM>(pnts.array(), xc, H,
                                                                 interp_pnts.array());
                                        for (size_t i = 0; i < HoChebNodes.cols(); i++)
                                        {
                                            auto coneId =
                                                domain.elementForPoint(interp_pnts.col(i));
                                            auto coneId2 =
                                                coarseDomain.elementForPoint(interp_pnts.col(i));
                                            if (coneId2 < SIZE_MAX)
                                                is_cone_active[1].insert(coneId2);
                                            if (coneId < SIZE_MAX)
                                                is_cone_active[0].insert(coneId);
                                ...
                                else
                                {
                                    ...
                                    for (size_t el : p_grid.activeCones())
                                    {
                                        for (size_t i = 0; i < chebNodes.cols(); i++)
                                        {
                                            Point cart_pnt = Util::interpToCart<DIM>(
                                                p_grid.transform(el, chebNodes.col(i)).array(), pxc,
                                                pH);
                                            Point interp_pnt =
                                                Util::cartToInterp<DIM>(cart_pnt, xc, H);
                        ...
                        /*
                        if(mode==TwoGrid) {
                            //mark all children of active elements in the coarse grid also as
                        active.
                        ...
                        for (int step = 0; step < N_STEPS; step++)
                        {
                            ...
                            for (size_t i : is_cone_active[step])
                            {
                                ...
                                { // We dont need the active cone map if we are in the recursive
                                  // stage
                            ...
                            // std::cout<<"level"<<node->level()<<"range:"<<box.min().transpose()<<"
            ...
            {
                tbb::queuing_mutex::scoped_lock lock(activeConeMutex);
                std::cout << "level= " << level << " active cones" << activeCones[0].size()
                          << " full: " << numBoxes(level) * coneDomain(level, 0, 0).n_elements()
                          << std::endl;
                std::cout << "level= " << level << " active cones" << activeCones[1].size()
                          << " full: " << numBoxes(level) * coneDomain(level, 0, 1).n_elements()
                          << std::endl;
    ...
    FieldInfo computeFieldInfo(int level, const Octree& target, bool isFarField = true) const
    {
        ...
        {
            std::shared_ptr<OctreeNode> node = m_nodes[level][n];
            const std::vector<IndexRange>& targets =
                isFarField ? node->farTargets() : node->nearTargets();
    ...
    unsigned int numBoxes(unsigned int level) const { return m_numBoxes[level]; }
    ...
    const auto point(size_t id) const { return m_pnts.col(id); }
    ...
    const bool isLeaf(unsigned int level, size_t i) const { return m_nodes[level][i]->isLeaf(); }
    ...
    size_t numLeafCones(size_t level) const { return m_leafCones[level].size(); }

    ConeRef leafCone(size_t level, size_t num) const { return m_leafCones[level][num]; }
    ...
    size_t numPoints() const { return m_pnts.size(); }
    ...
    void sanitize()
    {
        ...
        for (int level = 0; level < m_levels; level++)
        {
            ...
            for (int i = 0; i < indices.size(); i++)
            {
                if (indices[i] != 1)
                {
                    std::cout << "wrong" << indices[i] << " " << i << " level" << level
                              << std::endl;
  ...
  private:
    std::shared_ptr<OctreeNode> buildOctreeNode(std::shared_ptr<OctreeNode> parent,
                                                const IndexRange& pnt_range,
                                                ...
                                                unsigned int level = 0)
    {
        ...
        for (int j = 0; j < N_Children; j++)
        {
            ...
            // std::cout<<"building child "<<j<<" at"<<child_bbox.min().transpose()<<"
            // "<<child_bbox.max().transpose()<<std::endl;
    ...
    const auto& points() const { return m_pnts; }
  ...
  private:
    ...
    std::vector<FieldInfo> m_farFieldBoxes;  // on each level: for each target point y store the
                                             // source boxes such that y is in the farfield
    std::vector<FieldInfo> m_nearFieldBoxes; // on each level: for each target point y store the
                                             // source boxes such that y is in the farfield

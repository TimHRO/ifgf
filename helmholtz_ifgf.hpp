...
template <size_t dim>
class HelmholtzIfgfOperator
    : public IfgfOperator<std::complex<double>, dim, 1, HelmholtzIfgfOperator<dim>>
{
  public:
    typedef Eigen::Array<double, dim, Eigen::Dynamic> PointArray;
    typedef Eigen::Vector<double, dim> Point;
    HelmholtzIfgfOperator(double waveNumber, size_t leafSize, size_t order, size_t n_elem = 1,
                          double tol = -1)
        : IfgfOperator<std::complex<double>, dim, 1, HelmholtzIfgfOperator<dim>>(leafSize, order,
                                                                                 n_elem, tol),
          k(waveNumber)
    {
    }
                           ...
                           const Point& xc, double H, IndexRange srcsIds) const
    {
        ...
        for (int j = 0; j < y.cols(); j++)
        {
            ...
            {
                d = (x.middleCols(pkg_size * i, pkg_size).colwise() - y.col(j))
                        .matrix()
                        .colwise()
                        .norm()
                        .array();
                ...
                result[j] +=
                    (weights.segment(pkg_size * i, pkg_size).matrix().transpose() * tmp.matrix())
                        .value(); // *  std::complex<double>(  exp(std::complex<float>(0,(float) k)
                                  // * (d - dc)) * (dc) / d);
            }

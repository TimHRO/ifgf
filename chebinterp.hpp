...
template <typename T> Eigen::Array<T, Eigen::Dynamic, 1> cachedChebnodes1d(int n)
{
...
{
    const Eigen::Array<T, N_AT_COMPILE_TIME, 1> nodes1d = chebnodes1d<T, N_AT_COMPILE_TIME>(ns[0]);
...
{
    const Eigen::Array<T, NR_AT_COMPILE_TIME, 1> nodesR = chebnodes1d<T, NR_AT_COMPILE_TIME>(ns[0]);
    const Eigen::Array<T, NTHETA_AT_COMPILE_TIME, 1> nodesTheta =
        chebnodes1d<T, NTHETA_AT_COMPILE_TIME>(ns[1]);

    Eigen::Array<T, 2, Eigen::Dynamic> nodesNd(2, nodesR.size() * nodesTheta.size());
...
template <typename T, int NR_AT_COMPILE_TIME, int NTHETA_AT_COMPILE_TIME, int NPHI_AT_COMPILE_TIME>
inline Eigen::Array<T, 3, Eigen::Dynamic> chebnodesNd(const Eigen::Vector<int, 3>& ns)
{
    const Eigen::Array<T, NR_AT_COMPILE_TIME, 1> nodesR = chebnodes1d<T, NR_AT_COMPILE_TIME>(ns[0]);
    const Eigen::Array<T, NTHETA_AT_COMPILE_TIME, 1> nodesTheta =
        chebnodes1d<T, NTHETA_AT_COMPILE_TIME>(ns[1]);
    ...
    Eigen::Array<T, 3, Eigen::Dynamic> nodesNd(3,
                                               nodesR.size() * nodesTheta.size() * nodesPhi.size());
...
inline const Eigen::Array<T, Eigen::Dynamic, Eigen::Dynamic>& chebvals(size_t n)
{
    ...
    static std::unordered_map<size_t, Eigen::Array<T, Eigen::Dynamic, Eigen::Dynamic>> cache;
...
template <typename TV, typename TV2> auto computeDiffVector(int n, const TV& x, const TV2& nodes)
{
...
template <typename TV, typename TP> auto inline transformNodesToInterval(TV& nodes, TP min, TP max)
{
                   ...
                   const Eigen::Ref<const Eigen::Vector<int, DIM>>& ns)
{
    ...
    else
    {
        ...
        {
            // std::cout<<"idx"<<idx<<" "<<idx*stride<<" "<<M.size()<<" "<<src.size()<<"
            // "<<nsigma<<std::endl;
            chebtransform<T, DIM - 1>(src.segment(idx * stride, nsigma),
                                      M.segment(idx * stride, nsigma), ns.template head<DIM - 1>());
        }
...
template <typename T, int POINTS_AT_COMPILE_TIME, int DIM, int DIM_X, unsigned int DIMOUT,
          int ND = -1, int... Ns>
  ...
  public:
               ...
               const Eigen::Ref<const Eigen::Vector<int, DIM_X>>& ns)
    {
        ...
        else // recurse down
        {
            ...
            ClenshawEvaluator<T, POINTS_AT_COMPILE_TIME, std::max(DIM - 1, 1), DIM_X, DIMOUT, Ns...>
                clenshaw;
            if (Nd <= 2)
            {
                const Eigen::Array<T, POINTS_AT_COMPILE_TIME, DIMOUT>& c0 =
                    clenshaw(x, vals.middleRows(0 * stride, stride), ns).eval();
                ...
                {
                    b1 = clenshaw(x, vals.middleRows((1) * stride, stride), ns).eval();

                    return x.row(DIM - 1).transpose() * b1 + c0;
                }
            }

            b2 = clenshaw(x, vals.middleRows((Nd - 1) * stride, stride), ns).eval();
            const Eigen::Array<T, POINTS_AT_COMPILE_TIME, DIMOUT> cn2 =
                clenshaw(x, vals.middleRows((Nd - 2) * stride, stride), ns).eval();
            ...
            for (size_t j = Nd - 3; j > 0; j--)
            {
                tmp = clenshaw(x, vals.middleRows(j * stride, stride), ns);
...
evaluate(const Eigen::Ref<const Eigen::Array<PointScalarType, 3, Eigen::Dynamic>>& x,
         const Eigen::Ref<const Eigen::Array<T, Eigen::Dynamic, DIMOUT>>& vals,
         const Eigen::Vector<int, 3>& ns, BoundingBox<3> box = BoundingBox<3>())
{
    const int DIM = 3;
    ...
    Eigen::Array<PointScalarType, Eigen::Dynamic, Eigen::Dynamic> xdiff =
        computeDiffVector(ns[0], x.row(0), nodes1);
    Eigen::Array<PointScalarType, Eigen::Dynamic, Eigen::Dynamic> ydiff =
        computeDiffVector(ns[1], x.row(1), nodes2);
    Eigen::Array<PointScalarType, Eigen::Dynamic, Eigen::Dynamic> zdiff =
        computeDiffVector(ns[2], x.row(2), nodes3);
    ...
    // std::cout << "type diff = " << typeid(x(0,0)).name() << ", type vals = " <<
    // typeid(vals(0,0)).name() << std::endl;
...
template <typename T, typename PointScalarType, int DIMOUT>
inline Eigen::Array<T, Eigen::Dynamic, DIMOUT>
evaluate(const Eigen::Ref<const Eigen::Array<PointScalarType, 2, Eigen::Dynamic>>& x,
         const Eigen::Ref<const Eigen::Array<T, Eigen::Dynamic, DIMOUT>>& vals,
         const Eigen::Vector<int, 2>& ns, BoundingBox<2> box = BoundingBox<2>())
{
    const int DIM = 2;
    Eigen::Array<T, Eigen::Dynamic, DIMOUT> result(x.cols(), DIMOUT);
...
void fast_evaluate_tp(
    const Eigen::Ref<const Eigen::Array<double, DIM - 1, Eigen::Dynamic>>& points,
    const Eigen::Ref<const Eigen::Array<double, 1, Eigen::Dynamic>>& points2, int axis,
    const Eigen::Ref<const Eigen::Array<T, Eigen::Dynamic, DIMOUT>>& interp_values,
    Eigen::Ref<Eigen::Array<T, Eigen::Dynamic, DIMOUT>> dest, const Eigen::Vector<int, DIM>& ns,
    BoundingBox<DIM> box = BoundingBox<DIM>());

template <typename T, unsigned int DIM, unsigned int DIMOUT>
void tp_evaluate(const std::array<Eigen::Array<double, Eigen::Dynamic, 1>, DIM>& points,
                 const Eigen::Ref<const Eigen::Array<T, Eigen::Dynamic, DIMOUT>>& interp_values,
...
void parallel_evaluate(
    const Eigen::Ref<const Eigen::Array<double, DIM, Eigen::Dynamic>>& points,
    const Eigen::Ref<const Eigen::Array<T, Eigen::Dynamic, DIMOUT>>& interp_values,
    Eigen::Ref<Eigen::Array<T, Eigen::Dynamic, DIMOUT>> dest, const Eigen::Vector<int, DIM>& ns,
    BoundingBox<DIM> box = BoundingBox<DIM>());

template <int DIM> inline size_t cache_key(const Eigen::Ref<const Eigen::Vector<int, DIM>>& ns)
{
    size_t val = 0;
    for (int i = 0; i < DIM; i++)
    {
...
template <typename T, int DIM>
const Eigen::Array<T, DIM, Eigen::Dynamic>&
chebnodesNdd(const Eigen::Ref<const Eigen::Vector<int, DIM>>& ns)
{
...
template <typename T, int DIM, int DIMOUT> struct InterpolationData
{
    Eigen::Vector<int, DIM> order;
    ConeDomain<DIM> grid;
    Eigen::Array<T, Eigen::Dynamic, DIMOUT> values;

    size_t computeStride() const { return order.prod(); }
};

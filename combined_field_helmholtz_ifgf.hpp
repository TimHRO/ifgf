...
template <size_t dim>
class CombinedFieldHelmholtzIfgfOperator
    : public IfgfOperator<std::complex<double>, dim, 1, CombinedFieldHelmholtzIfgfOperator<dim>>
{
  public:
    typedef Eigen::Array<double, dim, Eigen::Dynamic> PointArray;
    typedef Eigen::Vector<double, dim> Point;
    CombinedFieldHelmholtzIfgfOperator(double waveNumber, size_t leafSize, size_t order,
                                       size_t n_elem = 1, double tolerance = -1)
        : IfgfOperator<std::complex<double>, dim, 1, CombinedFieldHelmholtzIfgfOperator<dim>>(
                            ...
                            const Eigen::Ref<const Point>& n) const
    {
        ...
        auto kern = exp(T(0,kappa)*d) / (4 * M_PI * d2*d)
            * ( nxy * (std::complex<double>(1.,0)*T(1.) - std::complex<double>(0,kappa)*d)  -
        T(0,kappa)*d2);
        ...
        for (int j = 0; j < y.cols(); j++)
        {
            ...
            for (int i = 0; i < x.cols(); i++)
            {
                ...
                if (d2 > 1e-14)
                {
                    ...
                    result.row(j).real() +=
                        f * ((w_r[i] * c - w_i[i] * s) * nxy +
                             (w_r[i] * s + w_i[i] * c) * kappa * (d * nxy + d2));
                    result.row(j).imag() +=
                        f * (-(w_r[i] * c - w_i[i] * s) * kappa * (d * nxy + d2) +
                             (w_r[i] * s + w_i[i] * c) * nxy);
                }

                // result.row(j) += weights[i]* ( exp(T(0, kappa) * (d - dc)) * dc / (d2*d)
                //                               * ( nxy * (std::complex<double>(1.,0)*T(1.)
                //                               - std::complex<double>(0,kappa)*d)  -
                //                               T(0,kappa)*d*d));
            }
        }

...
template <size_t dim>
class ModifiedHelmholtzIfgfOperator
    : public IfgfOperator<std::complex<double>, dim, 1, ModifiedHelmholtzIfgfOperator<dim>>
{
  public:
    typedef Eigen::Array<double, dim, Eigen::Dynamic> PointArray;
    typedef Eigen::Vector<double, dim> Point;
    ModifiedHelmholtzIfgfOperator(std::complex<double> waveNumber, size_t leafSize, size_t order,
                                  size_t n_elem = 1, double tol = -1)
        : IfgfOperator<std::complex<double>, dim, 1, ModifiedHelmholtzIfgfOperator<dim>>(
              leafSize, order, n_elem, tol),
          k(waveNumber)
    {
    }
                           ...
                           const Point& xc, double H, IndexRange srcsIds) const
    {
        ...
        for (int j = 0; j < y.cols(); j++)
        {
            ...
            for (int i = 0; i < x.cols(); i++)
            {
                ...
                result[j] += (d == 0) ? 0 : weights[i] * exp(-k * (d - dc)) * (dc) / d;
            }
        }
        return result;
    }
